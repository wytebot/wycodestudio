...
import{getFirestore,addDoc,collection,deleteDoc,doc,onSnapshot,query,serverTimestamp,updateDoc}from"firebase/firestore";
import{initializeApp}from"firebase/app";
import{LayoutDashboard,Package,ShoppingBag,Users,Settings as SettingsIcon,Plus,Search,LogOut,Pencil,Trash2,X,LockKeyhole,AlertTriangle}from"lucide-react";
import"./styles.css";
...
const blank={name:"",slug:"",description:"",category:"Developer Tools",price:9.99,status:"draft",version:"1.0.0",demoUrl:"",driveFileId:"",coverUrl:"",features:"",requirements:"",license:"Single-project commercial license"};
...
function App(){
 ...
 async function save(e){
   ...
   if(!name||!String(f.get("description")||"").trim()||!Number.isFinite(price)||price<0||!String(f.get("version")||"").trim()){setMsg("Name, description, valid non-negative price, and version are required.");return}
   const demoUrl=String(f.get("demoUrl")||"").trim(),coverUrl=String(f.get("coverUrl")||"").trim();
   if(!validUrl(demoUrl)||!validUrl(coverUrl)){setMsg("Demo URL and cover URL must be valid http/https URLs.");return}
   const d={name,slug:productSlug,description:String(f.get("description")||"").trim(),category:f.get("category"),price,status:f.get("status"),version:String(f.get("version")||"").trim(),demoUrl,driveFileId:String(f.get("driveFileId")||"").trim(),coverUrl,features:String(f.get("features")||"").trim(),requirements:String(f.get("requirements")||"").trim(),license:String(f.get("license")||"").trim(),updatedAt:serverTimestamp()};
   setLoading(true);
   try{
 ...
 async function remove(id){
   ...
   setLoading(true);try{await deleteDoc(doc(db,"products",id));setMsg("Product deleted.")}catch(e){setMsg(errText(e))}finally{setLoading(false)}
 }
 ...
 const revenue=orders.filter(x=>x.status==="paid").reduce((s,x)=>s+Number(x.amount||0),0),list=products.filter(x=>(x.name||"").toLowerCase().includes(search.toLowerCase())||(x.category||"").toLowerCase().includes(search.toLowerCase()));
...
function Dashboard({p,o,c,r}){return <><div className="cards"><Card t="Products" v={p.length}/><Card t="Paid orders" v={o.filter(x=>x.status==="paid").length}/><Card t="Customers" v={c.length}/><Card t="Revenue" v={money(r)}/></div><div className="grid"><section className="panel"><h2>Recent products</h2>{p.length?p.slice(0,6).map(x=><div className="row" key={x.id}><span><b>{x.name}</b><small>{x.category}</small></span><b>{money(x.price)}</b></div>):<p>No products yet.</p>}</section><section className="panel"><h2>Recent orders</h2>{o.length?o.slice(0,6).map(x=><div className="row" key={x.id}><span><b>{x.productName||"Order"}</b><small>{x.email||"—"}</small></span><b>{x.status}</b></div>):<p>No orders yet.</p>}</section></div></>}
function Card({t,v}){return <div className="card"><small>{t}</small><strong>{v}</strong></div>}
function Table({p,edit,remove}){return <section className="panel table"><div className="tr head"><span>Product</span><span>Price</span><span>Status</span><span>Drive</span><span/></div>{p.length?p.map(x=><div className="tr" key={x.id}><span><b>{x.name}</b><small>{x.slug}</small></span><span>{money(x.price)}</span><span className="pill">{x.status}</span><span>{x.driveFileId?"✓":"!"}</span><span><button onClick={()=>edit(x)} aria-label={`Edit ${x.name}`}><Pencil size={15}/></button><button onClick={()=>remove(x.id)} aria-label={`Delete ${x.name}`}><Trash2 size={15}/></button></span></div>):<p>No products match your search.</p>}</section>}
...
function Modal({p,close,save}){const fields=["name","slug","description","price","version","demoUrl","driveFileId","coverUrl","features","requirements","license"];return <div className="back"><form className="modal" onSubmit={save}><div className="modalhead"><h2>{p.id?"Edit product":"Add product"}</h2><button type="button" onClick={close} aria-label="Close"><X/></button></div><div className="form">{fields.map(k=><label key={k}>{k==="driveFileId"?"Google Drive file ID":k}<input name={k} defaultValue={p[k]??""} required={["name","description","price","version"].includes(k)} type={k==="price"?"number":"text"} min={k==="price"?"0":undefined} step={k==="price"?"0.01":undefined}/></label>)}<label>Status<select name="status" defaultValue={p.status}><option>draft</option><option>published</option><option>archived</option></select></label><label>Category<select name="category" defaultValue={p.category}><option>Developer Tools</option><option>Productivity</option><option>Business</option><option>Creative</option><option>Utilities</option><option>Other</option></select></label></div><button className="primary" type="submit">Save product</button></form></div>}
