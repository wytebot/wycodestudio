...
function Settings({user}){
 const[drive,setDrive]=useState(null),[checking,setChecking]=useState(false),[testSize,setTestSize]=useState(3*1024*1024);
 async function checkDrive(){
  ...
  try{
   ...
   const res=await fetch(`/api/drive-status?bytes=${encodeURIComponent(testSize)}`,{headers:{Authorization:`Bearer ${token}`}});
   const j=await res.json().catch(()=>({}));
   if(!res.ok)throw Object.assign(new Error(j.error||"Google Drive connection check failed"),{code:j.code});
   setDrive({state:"ok",message:j.message||"Google Drive is connected and ready.",storage:j.storage,account:j.account,folder:j.folder,readiness:j.uploadReadiness});
  }catch(e){setDrive({state:"error",message:e.message||"Google Drive connection check failed"});}
  finally{setChecking(false)}
 }
 return <section className="panel"><h2>Studio settings</h2><p>Admin: <b>{user.email}</b></p><div className="security"><LockKeyhole size={20}/><span>Product source files and cover images are stored in Google Drive. Firebase Storage is not used by this Studio.</span></div><div className="driveStatusPanel"><div className="driveStatusHead"><div><div className="driveStatusTitle"><Cloud size={19}/> Google Drive status</div><small>Live diagnostics for the account and upload destination.</small></div><button type="button" className="secondary" onClick={checkDrive} disabled={checking}>{checking?"Checking…":"Refresh status"}</button></div><div className="driveGrid"><div><span>Connected Google account</span><b>{drive?.account||"Not checked"}</b></div><div><span>Destination folder</span><b>{drive?.folder?.name||"Not checked"}</b><small>{drive?.folder?.id||"—"}</small></div><div><span>Storage / quota</span><b>{drive?.storage?.sharedDrive?"Shared Drive":drive?.storage?.limit>0?`${(drive.storage.free/(1024**3)).toFixed(2)} GB free`:"Unavailable"}</b><small>{drive?.storage?.sharedDrive?"Personal My Drive quota does not apply":"Checked before uploads"}</small></div><div><span>Upload readiness</span><b className={drive?.readiness?.status==="blocked"?"statusBad":"statusGood"}>{drive?.readiness?.status==="blocked"?"Blocked":drive?.readiness?.status==="ready"?"Ready":"Not checked"}</b><small>Test size: {(testSize/(1024*1024)).toFixed(0)} MB</small></div></div><label className="driveTestSize">Diagnostic upload size<select value={testSize} onChange={e=>{setTestSize(Number(e.target.value));setDrive(null)}}><option value={1024*1024}>1 MB</option><option value={3*1024*1024}>3 MB — max source upload</option><option value={10*1024*1024}>10 MB</option><option value={50*1024*1024}>50 MB</option></select></label>{drive?.readiness&&<div className={`driveReason ${drive.readiness.status==="blocked"?"bad":"good"}`}><b>{drive.readiness.status==="blocked"?"Why this upload would fail":"Why this upload is currently allowed"}</b><span>{drive.readiness.reason}</span></div>}{drive?.message&&<details className="driveDetails"><summary>Technical connection details</summary><p>{drive.message}</p></details>}</div><details><summary>Privacy</summary><p>This private admin tool processes only information needed to operate the marketplace. Credentials and private source files should remain server-side.</p></details><details><summary>Terms</summary><p>Studio access is restricted to the authorized administrator. Keep credentials private and use the dashboard only to manage your own marketplace data.</p></details><details><summary>About</summary><p>WyCode Studio is the private administration panel for the personal WyCode source-code marketplace.</p></details></section>}
